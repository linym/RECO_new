...
s6a_up_loc::s6a_ula_cb(
  ...
  if (avp_p) {
    ...
    memcpy(s6a_update_location_ans_p->imsi, \
           hdr_p->avp_value->os.data, hdr_p->avp_value->os.len);
   ...
   */
  CHECK_FCT(fd_msg_search_avp(ans_p, \
            s6a_fd_cnf.dataobj_s6a_result_code, &avp_p));
  ...
  if (avp_p) {
    ...
    MSC_LOG_TX_MESSAGE(MSC_S6A_MME, MSC_MMEAPP_MME, \
                       NULL, 0, "0 S6A_UPDATE_LOCATION_ANS imsi %s %s", \
     ...
     */
    CHECK_FCT(fd_msg_search_avp(ans_p, \
              s6a_fd_cnf.dataobj_s6a_experimental_result, &avp_p));
    ...
    if (avp_p) {
      ...
      s6a_parse_experimental_result(avp_p, &s6a_update_location_ans_p\
                                    ->result.choice.experimental);
      goto err;
    } else {
      /*
       ...
       */
      OAILOG_ERROR(LOG_S6A, "Experimental-Result and "
                   "Result-Code are absent: " \
  ...
  if (avp_p) {
    ...
    if (!FLAG_IS_SET(hdr_p->avp_value->u32, ULA_SEPARATION_IND)) {
      OAILOG_ERROR(LOG_S6A, "ULA-Flags does not indicate "
                   "the HSS is post Rel.8: " \
     ...
     */
    OAILOG_ERROR(LOG_S6A, "ULA-Flags AVP is absent while "
                 "result code indicates " "DIAMETER_SUCCESS\n");
    goto err;
  }

  CHECK_FCT(fd_msg_search_avp(ans_p, \
            s6a_fd_cnf.dataobj_s6a_subscription_data, &avp_p));

  if (avp_p) {
    s6a_up_loc ul;
    CHECK_FCT(ul.s6a_parse_subscription_data(avp_p, \
              &s6a_update_location_ans_p->subscription_data));
...
int
s6a_up_loc::s6a_generate_update_location(
  ...
  CHECK_FCT(fd_msg_avp_new(s6a_fd_cnf.\
            dataobj_s6a_auth_session_state, 0, &avp_p));
  /*
   * No State maintained
   */
  value.i32 = 1;
   ...
   */
  {
    bstring  host = bstrcpy(s6a_config.hss_host_name);

    bconchar(host, '.');
    bconcat(host, mme_config.realm);

    CHECK_FCT(fd_msg_avp_new(s6a_fd_cnf.\
              dataobj_s6a_destination_host, 0, &avp_p));
  ...
  {
    CHECK_FCT(fd_msg_avp_new(s6a_fd_cnf.\
              dataobj_s6a_destination_realm, 0, &avp_p));
   ...
   */
  {
    ...
    CHECK_FCT(fd_msg_avp_new(s6a_fd_cnf.\
              dataobj_s6a_visited_plmn_id, 0, &avp_p));
