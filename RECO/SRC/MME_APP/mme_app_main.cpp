...
public:
	...
	void AsyncCompleteRpc() 
    	{
		std::cout << "Enter AsyncCompleteRpc" << std::endl;
        	void* got_tag;
        	bool ok = false;
        	
        	while (cq_.Next(&got_tag, &ok)) 
        	{
			std::cout << "Enter AsyncCompleteRpc22222" << std::endl;
				...
				if (call->reply.base() == test::Reply_Base_DIAMETER_SUCCESS) ula_pP->result.choice.base = DIAMETER_SUCCESS;
				strcpy(ula_pP->imsi, call->reply.imsi().c_str());

				if (call->reply.subscriber_status() == test::Reply_subscriber_status_t_SS_SERVICE_GRANTED) ula_pP->subscription_data.subscriber_status = SS_SERVICE_GRANTED;
				else if (call->reply.subscriber_status() == test::Reply_subscriber_status_t_SS_OPERATOR_DETERMINED_BARRING) ula_pP->subscription_data.subscriber_status = SS_OPERATOR_DETERMINED_BARRING;
				else if (call->reply.subscriber_status() == test::Reply_subscriber_status_t_SS_MAX) ula_pP->subscription_data.subscriber_status = SS_MAX;
				...
				if (call->reply.access_mode() == test::Reply_access_mode_t_NAM_PACKET_AND_CIRCUIT) ula_pP->subscription_data.access_mode = NAM_PACKET_AND_CIRCUIT;
				else if (call->reply.access_mode() == test::Reply_access_mode_t_NAM_RESERVED) ula_pP->subscription_data.access_mode = NAM_RESERVED;
				else if (call->reply.access_mode() == test::Reply_access_mode_t_NAM_ONLY_PACKET) ula_pP->subscription_data.access_mode = NAM_ONLY_PACKET;
				else if (call->reply.access_mode() == test::Reply_access_mode_t_NAM_MAX) ula_pP->subscription_data.access_mode = NAM_MAX;		

				ula_pP->subscription_data.apn_config_profile.context_identifier = call->reply.context_identifier();
		
				if (call->reply.all_apn_conf_ind() == test::Reply_all_apn_conf_ind_t_ALL_APN_CONFIGURATIONS_INCLUDED) ula_pP->subscription_data.apn_config_profile.all_apn_conf_ind = ALL_APN_CONFIGURATIONS_INCLUDED;
		  		else if (call->reply.all_apn_conf_ind() == test::Reply_all_apn_conf_ind_t_MODIFIED_ADDED_APN_CONFIGURATIONS_INCLUDED) ula_pP->subscription_data.apn_config_profile.all_apn_conf_ind = MODIFIED_ADDED_APN_CONFIGURATIONS_INCLUDED;
		  		else if (call->reply.all_apn_conf_ind() == test::Reply_all_apn_conf_ind_t_ALL_APN_MAX) ula_pP->subscription_data.apn_config_profile.all_apn_conf_ind = ALL_APN_MAX;
				...
				if (call->reply.pdn_type() == test::Reply_pdn_type_t_IPv4) ula_pP->subscription_data.apn_config_profile.apn_configuration[0].pdn_type = IPv4;
		  		else if (call->reply.pdn_type() == test::Reply_pdn_type_t_IPv6) ula_pP->subscription_data.apn_config_profile.apn_configuration[0].pdn_type = IPv6;
		  		else if (call->reply.pdn_type() == test::Reply_pdn_type_t_IPv4_AND_v6) ula_pP->subscription_data.apn_config_profile.apn_configuration[0].pdn_type = IPv4_AND_v6;
		  		else if (call->reply.pdn_type() == test::Reply_pdn_type_t_IPv4_OR_v6) ula_pP->subscription_data.apn_config_profile.apn_configuration[0].pdn_type = IPv4_OR_v6;
		  		else if (call->reply.pdn_type() == test::Reply_pdn_type_t_IP_MAX) ula_pP->subscription_data.apn_config_profile.apn_configuration[0].pdn_type = IP_MAX;
				...
				if (call->reply.pre_emp_vulnerability() == test::Reply_pre_emption_vulnerability_t__PRE_EMPTION_VULNERABILITY_ENABLED) ula_pP->subscription_data.apn_config_profile.apn_configuration[0].subscribed_qos.allocation_retention_priority.pre_emp_vulnerability = (pre_emption_vulnerability_t)PRE_EMPTION_VULNERABILITY_ENABLED;
				else if (call->reply.pre_emp_vulnerability() == test::Reply_pre_emption_vulnerability_t__PRE_EMPTION_VULNERABILITY_DISABLED) ula_pP->subscription_data.apn_config_profile.apn_configuration[0].subscribed_qos.allocation_retention_priority.pre_emp_vulnerability = (pre_emption_vulnerability_t)PRE_EMPTION_VULNERABILITY_DISABLED;
				else if (call->reply.pre_emp_vulnerability() == test::Reply_pre_emption_vulnerability_t__PRE_EMPTION_VULNERABILITY_MAX) ula_pP->subscription_data.apn_config_profile.apn_configuration[0].subscribed_qos.allocation_retention_priority.pre_emp_vulnerability = (pre_emption_vulnerability_t)PRE_EMPTION_VULNERABILITY_MAX;


				if (call->reply.pre_emp_capability() == test::Reply_pre_emption_capability_t__PRE_EMPTION_CAPABILITY_ENABLED) ula_pP->subscription_data.apn_config_profile.apn_configuration[0].subscribed_qos.allocation_retention_priority.pre_emp_capability = (pre_emption_capability_t)PRE_EMPTION_CAPABILITY_ENABLED;
				else if (call->reply.pre_emp_capability() == test::Reply_pre_emption_capability_t__PRE_EMPTION_CAPABILITY_DISABLED) ula_pP->subscription_data.apn_config_profile.apn_configuration[0].subscribed_qos.allocation_retention_priority.pre_emp_capability = (pre_emption_capability_t)PRE_EMPTION_CAPABILITY_DISABLED;
				else if (call->reply.pre_emp_capability() == test::Reply_pre_emption_capability_t__PRE_EMPTION_CAPABILITY_MAX) ula_pP->subscription_data.apn_config_profile.apn_configuration[0].subscribed_qos.allocation_retention_priority.pre_emp_capability = (pre_emption_capability_t)PRE_EMPTION_CAPABILITY_MAX;	
	...
	/*void get_thread() {
		pthread_t t;
		pthread_create(&t, NULL, AsyncCompleteRpc, NULL);
		//return std::thread([=] {AsyncCompleteRpc();});
	}*/
...
std::string service_table(int user_type, char user_imsi[16]){
  MYSQL mysql;
  MYSQL_RES *res;
  MYSQL_ROW row;
  char query[300];
  ...
  mysql_close(&mysql);
  return std::string(service_ip_port);
}

int psuedo_identifier(){
  int user_type = 1;
  std::cout << "User type: 0=Human, 1=iot" << std::endl;
    ...
    case NAS_PDN_CONFIG_REQ: {
        ...
        if (ue_context_p) {
          if (!ue_context_p->is_s1_ue_context_release) {
         	//gPRC
		std::cout << "Client Send request." << std::endl;
		...
		//get user_type
		int user_type = psuedo_identifier();
		
		//get service location
		std::string service_ip_port;
		service_ip_port = service_table(user_type, user_imsi);
		...
		//create thread
		//std::thread thread_ = std::thread(&TestClient::AsyncCompleteRpc, &client);
		//client.get_thread();			
		//thread_.detach();
		typedef void *(*THREADFUNCPTR)(void *);
		pthread_t t;
		pthread_create(&t, NULL, (THREADFUNCPTR)&TestClient::AsyncCompleteRpc, &client);
	
		//send request to service server
		client.Send_request(ue_context_p, user_imsi);

		std::cout << "Client call FINISHED!!" << std::endl;
